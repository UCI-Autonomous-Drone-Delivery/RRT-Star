...
vector<Node*> Graph::nearestNeighborsGPU(Node* node, float r) {
	Coord* coord=node->coord;
	...
	y=coord->y;
	z=coord->z;
	cudaMallocManaged(&xArr, num_nodes*sizeof(float));
	cudaMallocManaged(&yArr, num_nodes*sizeof(float));
	cudaMallocManaged(&zArr, num_nodes*sizeof(float));
	cudaMallocManaged(&output, num_nodes*sizeof(bool));

	for(int i=0;i<num_nodes;i++) {
		xArr[i]=5;//adj_list.at(i)->coord->x;
		yArr[i]=5;//adj_list.at(i)->coord->y;
		zArr[i]=5;//adj_list.at(i)->coord->z;
	}
...
//	graph.printCellPop();
	...
	vector<Node*> neighbors=graph.nearestNeighborsGPU(n,50);
    auto t2 = Clock::now();
